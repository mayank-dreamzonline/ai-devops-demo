...
2. Restore the node group to healthy:
   `terraform -chdir=terraform/eks apply -auto-approve -var fault_active=false`
...
4. `terraform -chdir=terraform/eks destroy -auto-approve` — the EKS
...
on `vpc`/`eks`, add its destroy step between 3 and 4 (destroy anything
...
re-provisioning `vpc`/`eks` from scratch costs a real ~10-15 minute EKS
control-plane wait.
